...
MAINMENUSCREEN MACRO
    MoveCursor 24,8
    DisplayString STRCHAT1
    MoveCursor 24,10
    DisplayString STRCHAT2
    MoveCursor 24,12
    DisplayString STRCHAT3 
    MoveCursor 0,22
    DisplayHashedLine
...
column DB ?
row DB ?
STRCHATEND DB "To end Chatting Press F3", '$'
STRCHAT1 DB "To start chatting press F1 $" 
STRCHAT2 DB "To start game press F2 $" 
STRCHAT3 DB "To end program press ESC $"
STR8 DB "THANK YOU FOR PLAYING OUR GAME! THE END :)  $" 
LINEU1 DB ?
LINEU2 DB ?
MAXCHARS DB ?
MODE DB ?
...
ENDOFLEVEL1: ;Return to main menu
ClearScreen2
MOV MODE,4
JMP ENDLEVEL1COMPLETELY
...
ENDOFLEVEL2: ;Return to main menu
ClearScreen2
MOV MODE,4
JMP ENDLEVEL2COMPLETELY
...
gun1 proc 
    ...
    mov tjump,cl ;tjump system minute
    mov timedone,DH
    CMP timedone,50d
    JG AFTERMINUTE
    ADD timedone,10d
    ...
    MOV AH,2Ch ;get the system time
    INT 21h    ;CH = hour CL = minute DH = second DL = 1/100 seconds
    cmp DL,t ;DL has seconds and initialized by 0
    je start_gun
    mov t,DL ;move system seconds in t
    cmp DH,timedone ;tjump has system minute and cl has present minute
...
SCREEN2 PROC 
    ...
    check28: 
     GetKey INPUT_KEY,ASCII   
     CMP INPUT_KEY,59 ;CHECK IF USER CLICKED F1
    je F1_INPUT    
     CMP INPUT_KEY,60 ;CHECK IF USER CLICKED F2
    je F2_INPUT  
    CMP INPUT_KEY,1 ;CHECK IF USER CLICKED ESC
    je ESC_INPUT
    inc cx
    loop check28
    ...
    MoveCursor 1,24
    DisplayString STRCHATEND

   ;chatting text
   MOV LINEU1,0 ;Line before starting line of text for user 1 (since we will increment it later)
   MOV LINEU2,12 ;Line before starting line of text for user 2 (since we will increment it later)
   
   USER1: 
   MOV MAXCHARS,76 ;Number of characters that we can enter per line
   CMP LINEU1,10 ;The last row at which the user can write without needing to scroll
   JNL SCROLL1
   INC LINEU1
   MoveCursor 4,LINEU1
   Input1:
   ...
   CMP INPUT_KEY,3DH ;Comparing the input scan code with the scan code of F3
   JE ENDCHAT ;Go to the end of this procedure
   DisplayString ASCII
   DEC MAXCHARS ;Decrements the number of characters that can be entered per line by one
   CMP MAXCHARS,0
   JNZ Input1
   ...
   DEC LINEU1 ;Decrements LINEU1 since there is now an empty line user 1 can write in
   JMP USER1
